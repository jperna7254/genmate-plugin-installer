...
The no-mistakes Document step never reads this file — it runs with `CLAUDE.md` neutralized, which is why the policy above is also stated in `document.instructions` in `.no-mistakes.yaml`, read only from the default branch. That block is the authoritative copy; the paragraph above summarizes it for the agents that do read this file. If the yaml changes, update this summary to match it.
